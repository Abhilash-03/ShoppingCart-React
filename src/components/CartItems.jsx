...
const CartItems = ({ item }) => {
  ...
  return (
    <li className="flex space-x-2 hover:ring-2 hover:scale-105 transition-all justify-between font-bold bg-gray-800 text-white lg:w-2/4 sm:w-3/4  m-auto p-3 rounded-xl my-3 relative">
    <div className="image">
        <img src={item.images} alt={item.title} className="h-[200px] w-[250px] rounded-xl hover:scale-110 transition-all" />
    </div>
    <div className="item_body font-serif text-xl w-2/4">
        <h1 className="title">{item.title}</h1>
        <h3 className="mt-2 font-serif">Price: ₹{item.price}</h3>
        <div className="quantity mt-4 space-x-2">
    ...
    <div className="quant_price mt-3">
