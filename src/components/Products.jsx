...
const Products = ({ items }) => {
  ...
  return (
    <div className="product md:w-3/5 lg:w-2/5 w-3/4 text-xl bg-slate-600 font-serif p-2 m-2 mt-4 rounded-xl overflow-hidden transition-all delay-500">
      <div className="flex flex-col xl:flex-row bg-gray-800 items-center justify-center  rounded-xl p-2 ">
         ...
         <button onClick={() => setShow(!show)} className="h-12 w-12 rounded-full text-2xl bg-slate-200 text-black p-2 font-bold hover:bg-slate-300 disabled:bg-gray-300 disabled:text-black"><i className="fa-solid fa-eye"></i></button>
         <button onClick={() => handleCartItems(items)} className="h-14 w-14 rounded-full text-2xl bg-yellow-500 text-black py-3 font-bold hover:bg-yellow-300 disabled:bg-gray-300 disabled:text-black absolute -top-24" disabled={isAdded ? true : false}>{isAdded ? <i className="fa-solid fa-cart-shopping"></i> : <i className="fa-solid fa-cart-plus"></i>}</button>
         </div>
         </div>
         {
          show &&
