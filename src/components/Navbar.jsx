...
const Navbar = () => {
  ...
  return (
    <nav className="h-14 w-full bg-gray-700 text-white flex justify-between px-5 items-center sticky top-0 z-10">
      <Link to='/' className="hover:text-red-400 "><h1 className="md:text-3xl  text-xl font-bold">Shopping<span className="text-yellow-400 font-serif">Cart</span></h1></Link>
      <SearchBar />
     ...
     <Link to='/cart/products'><button className="bg-black text-white p-2 h-12 w-12 font-bold text-2xl rounded-full ml-4 hover:bg-slate-600"><i className="fa-solid fa-cart-plus"></i> 
     { cartState.cartProduct.length > 0 &&
     <span className="absolute bg-teal-500 rounded-full h-5 w-5 text-sm top-1 text-black font-serif">{cartState.cartProduct.length}</span>
     }
     </button></Link>
...
{ isMobileMenu &&
     <ul className="absolute right-0 -bottom-20 top-14 backdrop-blur-sm text-white p-3 font-bold md:text-xl text-sm h-screen w-1/4 block lg:hidden transition-all text-center font-serif">
     <Link to='/'>
