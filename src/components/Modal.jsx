...
const Modal = ({ item, show, setShow }) => {
  ...
  return (
    <div className="lg:w-2/3 lg:m-auto mx-4 px-3 font-serif fixed lg:top-40 top-24 sm:top-20 left-0 right-0 flex items-center py-4 backdrop-blur-md text-white ring-2 ring-yellow-400 z-10 rounded-2xl">
      <div className="prod_body flex flex-col md:flex-row space-x-4 bg-gray-900 p-4 rounded-xl">
        <div className="image flex  items-center">
            <img src={item.images} alt={item.title} className="h-[250px] lg:w-[280px] w-full mb-3 rounded-xl" />
        </div>
        <div className="content font-bold md:w-2/3 lg:w-full space-y-3">
            <h1 className="lg:text-3xl text-2xl font-bold">{item.title}</h1>
            <h4 className="text-2xl">Price: ₹{item.price}</h4>
            <p className="text-lg md:text-xl">Description: {item.description}</p>
        </div>
